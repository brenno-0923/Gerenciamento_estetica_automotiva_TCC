    ...
    if ($resultado->num_rows > 0) {
        ...
        if ($resultAvailableDates === false || $resultAvailableTimes === false) {
            ...
            if (!empty($availableDates) && !empty($availableTimes)) {
                echo "<script>alert('O horário não está disponível, selecione outro. Para te auxiliar, aqui estão as datas  [" . implode(", ", $availableDates) . "] e horas  [" . implode(", ", $availableTimes) . "] INDISPONÍVES, selecione outro horário, lembrando que uma hora pode ter varios dias.💡');</script>";
        ...
        if (isset($_GET['id_cliente'])) {
            ...
            $resultUpdate = $conexao->query("UPDATE cliente SET data_cadastro = '$data_cadastro', horario = '$horario' WHERE id_cliente = '$id_cliente'");
            ...
            $result_cliente = $conexao->query("INSERT INTO cliente (data_cadastro, horario) VALUES ('$data_cadastro', '$horario')");
        ...
        }header("Location: dados-cliente.php?id_cliente=" . $id_cliente);
    }   
}
...
<script>
    ...
    function preencherDatasDisponiveis() {
        ...
        var dataFim = new Date("2024-12-31"); // Final de 2023
